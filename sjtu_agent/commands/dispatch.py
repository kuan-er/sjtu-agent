...
CommandHandler = Callable[[str, list[str]], CommandResult]
...
def run_command(text: str, user_id: str = "") -> CommandResult | None:
    """Execute a shared core command.

    Returns a structured :class:`CommandResult`, or ``None`` when the input
    is not a core slash command. Exceptions are converted to the same
    readable format Feishu used, so the Web endpoint never crashes on a
    bad command.
    """
    parsed = parse_command(text)
    if parsed is None:
    ...
    try:
        ...
        import traceback
        text = f"[命令错误] `{cmd}` 执行出错：{exc}\n```\n{traceback.format_exc()[-300:]}\n```"
        return CommandResult(view="markdown", text=text, data={"ok": False, "command": cmd, "error": str(exc)})
