...
默认模型 `deepseek-v4-flash`。DeepSeek 官方、OpenAI 等其他兼容接口可在 Web GUI 或 setup 中选择「自定义」。

### 配置视觉模型（可选）

主模型不支持视觉（如 `deepseek-v4-flash`）时，可额外配置 `qwen-vl-max` 等视觉模型用于识图。`sjtu-agent setup` 会引导配置，详见 [agent_config.example.json](agent_config.example.json)。
