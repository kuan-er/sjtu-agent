...
### 发图片，模型回复"看不到你的问题"或输出一串 `<｜｜DSML｜｜` 标记

先 `sjtu-agent update` 到最新版——旧版本有两个已修复的问题：

1. **图片消息被上下文折叠机制误删**：旧版把图片的 base64 数据按长度计入上下文预算，一张图就能触发折叠、把图片消息本身折叠掉，模型只会说"看不到对话"。已修复为图片按固定视觉成本计费，且最近几轮永不折叠。
2. **工具调用标记混入正文**：DeepSeek 流式偶发把工具调用以 DSML 标记原文输出（服务端行为）。新版会自动剥离并让模型重发，用户不会再看到原始标记。

另外注意：常驻 Bot **不会热加载**代码和配置——改完 `agent_config.json`（如切换主模型）或更新版本后，需要重启 Bot（`sjtu-agent daemons restart`，psmux 用户重开会话）才生效。
