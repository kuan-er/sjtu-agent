...
Default Base URL: `https://models.sjtu.edu.cn/api/v1`. Default model: `deepseek-v4-flash` (DeepSeek V3.2 hosted at SJTU).

Available models:

| Model ID | Description |
|----------|-------------|
| `deepseek-v4-flash` | DeepSeek V3.2 (**default**) |
...
If your main model does not support vision input (e.g. `deepseek-v4-flash`), you can configure a separate vision model (e.g. `qwen-vl-max`). When Feishu receives an image, the vision model recognizes it first, with OCR as a fallback. **Choose any one of three configuration methods:**
