...
## Unreleased
- 🌐 `web_search` 支持专用搜索代理：设置 `SJTU_WEB_SEARCH_PROXY` 后仅搜索引擎请求走代理，其余流量（校园 API/DDL/推送）完全直连，解决"服务器要代理才能搜索但又怕全局挂代理影响校园账号"的两难；服务器部署文档新增代理与搜索章节（HTTPS_PROXY / NO_PROXY / 专用代理三选一）
